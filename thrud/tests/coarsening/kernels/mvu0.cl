...
//#define CF 64
//
//__kernel void MatVecMulUncoalesced0(const __global float* restrict M,
//                                    const __global float* restrict V,
//                                    uint width, uint height,
//                                    __global float* restrict W) {
//  size_t ntid = get_global_id(0);
//  size_t otids [CF];
//
//  #pragma unroll
//  for (unsigned int CI = 0; CI < CF; ++CI)
//    otids[CI] = ntid * CF + CI;
//
//  float dotProduct[CF];
//  #pragma unroll
...
//    #pragma unroll
//    for (unsigned int CI = 0; CI < CF; ++CI)
//      dotProduct[CI] += M[otids[CI] * width + x] * V[x];
//  }
//
//  #pragma unroll
//  for (unsigned int CI = 0; CI < CF; ++CI)
//    W[otids[CI]] = dotProduct[CI];
//}
...
__kernel void MatVecMulUncoalesced0(const __global float* M,
                                    ...
                                    uint width, uint height,
                                    __global float* W) {
  uint y1 = 2 * get_global_id(0);
  uint y2 = y1 + 1;
  float dotProduct1 = 0;
  float dotProduct2 = 0;
  for (int x = 0; x < width; ++x) {
    dotProduct1 += M[y1 * width + x] * V[x];
    dotProduct2 += M[y2 * width + x] * V[x];
  }
  W[y1] = dotProduct1;
  W[y2] = dotProduct2;
}
